...
<?php
    ...
    $id=$_GET['id'];
    $userquery = "SELECT * FROM `ans` WHERE `ID` = $id";
    $userRunQuery = mysqli_query($conn, $userquery);
...
<?php } ?>
    <form class="m-5" action="handlers/handle-editAns.php?id=<?= $current_quest['ID'] ?>" METHOD="POST">
        <h1 class="mb-5">Edit Answer</h1>

        <label for="formGroupExampleInput" class="form-label font-weight-bold">Body</label>
        <div class="mb-1">
            <textarea name="content" class = "editor" cols="60" rows="5"><?= $current_quest['ansDetails'] ?></textarea>
        </div>
        <div class="mt-5">
            <button type="submit" name="submit" class="btn btn-primary">Edit Answer</button>
        </div>
    </form>
</div>
