...
if(!empty($_SESSION['userID']))
{
    ...
    $ansQuery="SELECT * FROM `ans` WHERE `user_id`=$userID";
    ...
    <div class="row py-3 my-5">
        ...
        <div class="col-md-10">
            <div class="row">
                ...
                <section id="Answers">
                    <div class="MyAnswers mt-4 ml-2">
                    


                    <?php 
                    $per_page=4;
                    $Query="SELECT * FROM `ans` WHERE `user_id`=$userID";
                    ...
                    $Query="SELECT * from `ans` WHERE `user_id`=$userID LIMIT $first_row, $per_page";
                    $runQuery=mysqli_query($conn, $Query) or die( mysqli_error($conn));

                    while($row = mysqli_fetch_array($runQuery)){?>
                        <?php echo $row['ansDetails'] ?>
                        <p small class="text-muted"><?php echo $row['created_at'] ?></p>
                        <a href="handlers/handle-delAns.php?id=<?= $row['ID'] ?>">Delete</a>
                        <a href="editAns.php?id=<?= $row['ID'] ?>">Edit</a><?php
                        echo "<p>_______________________________________________________</P>";
                    }

                    for($page=1; $page<=$numofPages; $page++){
                        echo '<a class="btn btn-primary" href="profile.php?page= '. $page .'">' . $page . '</a>';
                        echo " ";
                    }
                    ?> 
